...
contract FractionalAggregatePool is ERC20Permit, IFractionalAggregatePool {
    ...
    modifier tokenRegistered(address _tokenAddress) {
        require(registered[_tokenAddress] == true, "AggregatePool: Token not accepted in this pool.");
        _;
    }
     ...
     */
    function _refPricePerToken(address _tokenAddress) private view returns (uint256) {
        address nft = spFactory.getNftByPool(_tokenAddress);
        require(nft != address(0), "AggregatePool: Unrecognized separate pool provided");
