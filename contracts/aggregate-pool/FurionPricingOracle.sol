...
contract FurionPricingOracle is Ownable {
    ...
    function getPriceLevel(address _token, uint256 _id) public view returns (uint256) {
        return priceLevel[_token][_id];
    }

    // update price to all levels under certain token address
    function updatePrices(address _token, uint256[] memory _prices) public onlyOwner {
    ...
    function updatePrice(address _token, uint256 _level, uint256 _price) public onlyOwner {
