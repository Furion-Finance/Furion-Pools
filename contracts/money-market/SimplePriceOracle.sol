...
contract SimplePriceOracle is IPriceOracle {
    ...
    function _getUnderlyingAddress(address _fToken) private view returns (address asset) {
        if (compareStrings(IERC20MetadataUpgradeable(_fToken).symbol(), "fETH")) {
    ...
    function getUnderlyingPrice(address _fToken) public view override returns (uint256, uint256) {
        address asset = _getUnderlyingAddress(_fToken);
        return (prices[asset], decimals[asset]);
    }

    function setUnderlyingPrice(address _fToken, uint256 _underlyingPriceMantissa, uint256 _decimal) public {
        address asset = _getUnderlyingAddress(_fToken);
    ...
    function assetPrices(address _asset) external view returns (uint256, uint256) {
