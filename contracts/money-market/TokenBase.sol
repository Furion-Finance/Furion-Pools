...
abstract contract TokenBase is ERC20PermitUpgradeable, TokenBaseStorage, ITokenBase {
    function __TokenBase_init(
        ...
        require(IRiskManager(_riskManager).isRiskManager(), "TokenBase: Not risk manager contract");
     ...
     */
    function setPendingAdmin(address _newPendingAdmin) external override onlyAdmin {
    ...
    function setReserveFactor(uint256 _newReserveFactorMantissa) external onlyAdmin {
        (uint256 borrows, uint256 reserves, uint256 index) = accrueInfo();
        accrueInterest(index);
        totalBorrows = borrows;
        totalReserves = reserves;

        require(_newReserveFactorMantissa < RESERVE_FACTOR_MAX_MANTISSA, "TokenBase: Invalid reserve factor");
    ...
    function getAccountSnapshot(address _account) external view returns (uint256, uint256, uint256) {
        ...
        return (balanceOf(_account), borrowBalanceCalc(_account, index), exchangeRateCalc(borrows, reserves));
    }
    ...
    function supplyRatePerBlock() external view override returns (uint256) {
        ...
        return interestRateModel.getSupplyRate(totalCash, borrows, reserves, reserveFactorMantissa);
    }
     ...
     */
    function borrowBalanceCalc(address _account, uint256 _index) internal view returns (uint256) {
        // Get borrowBalance and borrowIndex
        BorrowSnapshot memory snapshot = accountBorrows[_account];
    ...
    function exchangeRateCurrent() public view returns (uint256) {
        ...
        if (_totalSupply == 0) {
             ...
             */
            uint256 cashPlusBorrowsMinusReserves = totalCash + borrows - reserves;
     ...
     */
    function exchangeRateCalc(uint256 _borrows, uint256 _reserves) internal view returns (uint256) {
        uint256 _totalSupply = totalSupply();
        if (_totalSupply == 0) {
            /*
             * If there are no tokens minted:
             ...
             *  exchangeRate = (totalCash + totalBorrows - totalReserves) / totalSupply
             */
            uint256 cashPlusBorrowsMinusReserves = totalCash + _borrows - _reserves;
            uint256 exchangeRate = (cashPlusBorrowsMinusReserves * expScale) / _totalSupply;
    ...
    function accrueInfo() internal view returns (uint256, uint256, uint256) {
        ...
        if (blockDelta > 0) {
            ...
            require(borrowRatePerBlockMantissa <= BORROW_RATE_MAX_MANTISSA, "TokenBase: Borrow rate is absurdly high");
            ...
            Exp memory simpleInterestFactor = mul_(Exp({ mantissa: borrowRatePerBlockMantissa }), blockDelta);
            index = mul_ScalarTruncateAddUInt(simpleInterestFactor, indexPrior, indexPrior);
    ...
    function supplyInternal(address _supplier, uint256 _supplyAmount) internal {
        ...
        require(riskManager.supplyAllowed(address(this)), "TokenBase: Supply disallowed by risk manager");

        // Get current exchange rate
        Exp memory exchangeRate = Exp({ mantissa: exchangeRateCalc(borrows, reserves) });
     ...
     */
    function redeemInternal(address _redeemer, uint256 _redeemTokens, uint256 _redeemAmount) internal {
        (uint256 borrows, uint256 reserves, uint256 index) = accrueInfo();
        accrueInterest(index);
        ...
        require(totalCash > redeemAmount, "TokenBase: Market has insufficient cash");
    ...
    function borrowInternal(address _borrower, uint256 _borrowAmount) internal {
        ...
        // Fail gracefully if protocol has insufficient cash
        require(totalCash > _borrowAmount, "TokenBase: Market has insufficient cash");

        // We calculate the new borrower and total borrow balances, failing on overflow
        uint256 borrowBalanceNew = borrowBalanceCalc(_borrower, index) + _borrowAmount;
        ...
        emit Borrow(_borrower, _borrowAmount, borrowBalanceNew, totalBorrowsNew);
    }
     ...
     */
    function repayBorrowInternal(address _payer, address _borrower, uint256 _repayAmount) internal {
        (uint256 borrows, , uint256 index) = accrueInfo();
        accrueInterest(index);

        require(riskManager.repayBorrowAllowed(address(this)), "TokenBase: Repay disallowed by risk manager");
        ...
        uint256 actualRepayAmount = _repayAmount == type(uint256).max ? borrowBalancePrev : _repayAmount;
         ...
         */
        uint256 borrowBalanceNew = actualRepayAmount > borrowBalancePrev ? 0 : borrowBalancePrev - actualRepayAmount;
        uint256 totalBorrowsNew = actualRepayAmount > borrows ? 0 : borrows - actualRepayAmount;
        ...
        emit RepayBorrow(_payer, _borrower, actualRepayAmount, borrowBalanceNew, totalBorrowsNew);
    }
    ...
    function liquidateBorrowInternal(
        ...
        require(
            riskManager.liquidateBorrowAllowed(address(this), _fTokenCollateral, _borrower, _repayAmount),
            "TokenBase: Liquidation disallowed by risk manager"
        );
        // Fail if borrower = liquidator
        require(_borrower != _liquidator, "TokenBase: Cannot liquidate yourself");
        // Fail if repayAmount = 0 or -1
        require(_repayAmount > 0 && _repayAmount != type(uint256).max, "TokenBase: Invalid repay amount");
        ...
        emit LiquidateBorrow(_liquidator, _borrower, _repayAmount, _fTokenCollateral);
    }
     ...
     */
    function seizeInternal(address _seizer, address _liquidator, address _borrower, uint256 _repayAmount) internal {
        // We calculate the number of collateral tokens that will be seized
        (uint256 seizeTotal, uint256 repayValue) = riskManager.liquidateCalculateSeizeTokens(
            _borrower,
            _seizer,
            address(this),
            _repayAmount
        ...
        (bool allowed, bool isCollateralTier) = riskManager.seizeAllowed(address(this), _seizer, _borrower, seizeTotal);
        ...
        if (isCollateralTier && checker.isFurionToken(address(this))) {
            ...
            bytes32 id = keccak256(abi.encodePacked(block.timestamp, _borrower, liquidationCount[_borrower]));
            ...
            (uint256 liquidatorSeizeTokens, uint256 protocolSeizeAmount, uint256 totalReservesNew) = seizeAllocation(
                seizeTotal
            );
     ...
     */
    function seize(address _liquidator, address _borrower, uint256 _repayAmount) external {
    ...
    function seizeAllocation(
        uint256 _seizeTotal
    ) internal view returns (uint256 liquidatorSeizeTokens, uint256 protocolSeizeAmount, uint256 totalReservesNew) {
        // mul_: uint, exp -> uint
        uint256 protocolSeizeTokens = mul_(_seizeTotal, Exp({ mantissa: protocolSeizeShareMantissa }));
        ...
        protocolSeizeAmount = mul_ScalarTruncate(exchangeRate, protocolSeizeTokens);
    ...
    function claimLiquidation(bytes32 _id) external {
        ...
        require(block.timestamp > uint256(lp.time) + 1 days, "TokenBase: Time limit not passed");
        require(lp.value != 0, "TokenBase: Liquidation protection closed / never existed");
        require(msg.sender == lp.liquidator, "TokenBase: Not liquidator of this liquidation");
    ...
    function repayLiquidationWithEth(bytes32 _id) external payable {
        ...
        require(block.timestamp < uint256(lp.time) + 1 days, "TokenBase: Time limit passed");
        require(lp.value != 0, "TokenBase: Liquidation protection closed / never existed");
        ...
        // div_: uint, exp -> uint
        uint256 EthToRepay = div_(valueAfterMultiplier, Exp({ mantissa: EthPriceMantissa }));
    ...
    function repayLiquidationWithErc(bytes32 _id, address _fToken) external {
        ...
        require(block.timestamp < uint256(lp.time) + 1 days, "TokenBase: Time limit passed");
        require(riskManager.checkListed(_fToken), "TokenBase: Market not listed");
        require(lp.value != 0, "TokenBase: Liquidation protection closed / never existed");
        require(msg.sender == lp.borrower, "TokenBase: Not borrower of this liquidation");
        ...
        // div_: uint, exp -> uint
        uint256 underlyingToRepay = div_(valueAfterMultiplier, Exp({ mantissa: underlyingPriceMantissa }));

        // Pay liquidator
        IERC20(underlyingAsset).transferFrom(msg.sender, lp.liquidator, underlyingToRepay);
     ...
     */
    function doTransferOut(address payable _to, uint256 _amount) internal virtual;
}
