...
contract RiskManager is Initializable, RiskManagerStorage, IRiskManager {
     ...
     */
    function checkMembership(address _account, address _fToken) external view returns (bool) {
     ...
     */
    function addToMarketInternal(address _fToken, address _borrower) internal onlyListed(_fToken) {
    ...
    function exitMarket(address _fToken) external override {
        ...
        (uint256 tokensHeld, uint256 amountOwed, ) = ITokenBase(_fToken).getAccountSnapshot(msg.sender);
        // Fail if the sender has a borrow balance
        require(amountOwed == 0, "RiskManager: Borrow balance is not zero");

        // Fail if the sender is not permitted to redeem all of their tokens
        require(redeemAllowed(_fToken, msg.sender, tokensHeld), "RiskManager: Cannot withdraw all tokens");
    ...
    function setCloseFactor(uint256 _newCloseFactorMantissa) external onlyAdmin {
    ...
    function setCollateralFactor(
        ...
        require(lessThanExp(newCollateralFactorExp, limit), "RiskManager: Collateral factor larger than limit");
     ...
     */
    function supportMarket(address _fToken, uint256 _collateralFactorMantissa, uint256 _tier) external onlyAdmin {
        require(!markets[_fToken].isListed, "RiskManager: Market already listed");
        require(_collateralFactorMantissa <= COLLATERAL_FACTOR_MAX_MANTISSA, "RiskManager: Invalid collateral factor");
    ...
    function setSupplyPaused(address _fToken, bool _state) external onlyListed(_fToken) onlyAdmin returns (bool) {
        supplyGuardianPaused[_fToken] = _state;
        emit ActionPausedMarket(_fToken, "Supply", _state);
        return _state;
    }

    function setBorrowPaused(address _fToken, bool _state) external onlyListed(_fToken) onlyAdmin returns (bool) {
     ...
     */
    function supplyAllowed(address _fToken) external view onlyListed(_fToken) returns (bool) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!supplyGuardianPaused[_fToken], "RiskManager: Supplying is paused");
    ...
    function redeemAllowed(
        ...
        (, uint256 shortfall, ) = getHypotheticalAccountLiquidity(_redeemer, _fToken, _redeemTokens, 0);
    ...
    function borrowAllowed(
        ...
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!borrowGuardianPaused[_fToken], "RiskManager: Borrow is paused");

        if (!markets[_fToken].isMember[_borrower]) {
            // only fToken contract may call borrowAllowed if borrower not in market
            require(msg.sender == _fToken, "RiskManager: Sender must be fToken contract");
        ...
        (, uint256 shortfall, ) = getHypotheticalAccountLiquidity(_borrower, _fToken, 0, _borrowAmount);
        require(shortfall == 0, "RiskManager: Shortfall created, cannot borrow");
     ...
     */
    function repayBorrowAllowed(address _fToken) external view onlyListed(_fToken) returns (bool) {
        return true;
    }
    ...
    function liquidateBorrowAllowed(
        ...
        require(initiationBlockNumber > 0, "RiskManager: Liquidation not yet initiated");

        require(
            markets[_fTokenBorrowed].isListed && markets[_fTokenCollateral].isListed,
        ...
        uint256 borrowBalance = ITokenBase(_fTokenBorrowed).borrowBalanceCurrent(_borrower);

        (, uint256 shortfall, uint256 highestBorrowTier) = getAccountLiquidity(_borrower);
        ...
        require(initiationBlockNumber + 60 > block.number, "RiskManager: Reset auction required");
    ...
    function seizeAllowed(
        ...
        require(
            IERC20Upgradeable(_fTokenCollateral).balanceOf(_borrower) >= _seizeTokens,
        ...
        require(
            ITokenBase(_fTokenCollateral).getRiskManager() == ITokenBase(_fTokenBorrowed).getRiskManager(),
    ...
    function transferAllowed(address _fToken, address _src, uint256 _amount) external view returns (bool) {
        ...
        require(redeemAllowed(_fToken, _src, _amount), "RiskManager: Source not allowed to redeem that much fTokens");
    ...
    function initiateLiquidation(address _account) external {
        ...
        require(
            initiationBlockNumber == 0 || block.number > initiationBlockNumber + 60,
     ...
     */
    function closeLiquidation(address _account) external onlyListed(msg.sender) {
        (, uint256 shortfall, ) = getAccountLiquidity(_account);
    ...
    function collateralFactorBoost(address _account) public view returns (uint256 boostMantissa) {
    ...
    function getAccountLiquidity(
        address _account
    ) public view returns (uint256[] memory liquidities, uint256 shortfall, uint256 highestBorrowTier) {
        // address(0) -> no iteractions with market
        (liquidities, shortfall, highestBorrowTier) = getHypotheticalAccountLiquidity(_account, address(0), 0, 0);
    }
    ...
    function getHypotheticalAccountLiquidity(
        ...
        for (uint256 i; i < assets.length; ) {
            ...
            (vars.tokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = ITokenBase(vars.asset)
            ...
            vars.collateralFactor = Exp({
                mantissa: markets[vars.asset].collateralFactorMantissa + collateralFactorBoost(_account)
            });

            vars.exchangeRate = Exp({ mantissa: vars.exchangeRateMantissa });

            // Get the normalized price of the underlying asset of fToken
            (vars.oraclePriceMantissa, vars.decimal) = oracle.getUnderlyingPrice(vars.asset);
            require(vars.oraclePriceMantissa > 0, "RiskManager: Oracle price is 0");
            vars.oraclePrice = Exp({ mantissa: vars.oraclePriceMantissa });

            // Pre-compute a conversion factor from tokens -> ether (normalized price value)
            vars.collateralValuePerToken = mul_(mul_(vars.oraclePrice, vars.exchangeRate), vars.collateralFactor);
            ...
            tierBorrowValues[vars.assetTier - 1] += (vars.borrowBalance * vars.oraclePriceMantissa) / vars.decimal;
            ...
            if (vars.asset == _fToken) {
                ...
                tierBorrowValues[vars.assetTier - 1] += (_borrowAmount * vars.oraclePriceMantissa) / vars.decimal;
            }
        ...
        for (uint256 i = vars.maxTierMem; i > 0; ) {
            ...
            vars.threshold = tierBorrowValues[i - 1] + vars.accumulatedShortfall;
    ...
    function liquidateCalculateSeizeTokens(
        ...
        (uint256 priceBorrowedMantissa, uint256 borrowedDecimal) = oracle.getUnderlyingPrice(_fTokenBorrowed);
        (uint256 priceCollateralMantissa, ) = oracle.getUnderlyingPrice(_fTokenCollateral);
        require(priceBorrowedMantissa > 0 && priceCollateralMantissa > 0, "RiskManager: Oracle price is 0");
        ...
        uint256 valueAfterDiscount = (priceBorrowedMantissa * amountAfterDiscount) / borrowedDecimal;

        // Stored version used because accrueInterest() already called at the
        // beginning of liquidateBorrowInternal()
        uint256 collateralExchangeRateMantissa = ITokenBase(_fTokenCollateral).exchangeRateCurrent(); // Note: reverts on error
     ...
     */
    function liquidateCalculateDiscount(address _borrower) public view returns (uint256 discountMantissa) {
        uint256 startBlock = liquidatableTime[_borrower];
        uint256 currentBlock = block.number;
        // Solidity rounds down result by default, which is fine
        uint256 discountIntervalPassed = (currentBlock - startBlock) / discountInterval;

        discountMantissa = LIQUIDATION_INCENTIVE_MIN_MANTISSA + discountIncreaseMantissa * discountIntervalPassed;
